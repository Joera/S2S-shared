...
function _mergeNamespaces$1(n3, m3) {
  ...
  switch (claims[ROLE_CLAIM]) {
    ...
    case graphql_star.Role.OnboardingUser:
      return types_star.ok({
        ...
        app: claims.aud,
        authenticationId: claims.sid,
        role: graphql_star.Role.OnboardingUser,
        signer: claims.sub,
        sponsored: claims[SPONSORED_CLAIM]
      });
    case graphql_star.Role.Builder:
      return types_star.ok({
        ...
        app: claims.aud,
        authenticationId: claims.sid,
        role: graphql_star.Role.Builder,
        signer: claims.sub,
        sponsored: claims[SPONSORED_CLAIM]
      });
    default:
      return types_star.err(UnexpectedError.from(`Unexpected role: ${claims[ROLE_CLAIM]}`));
  }
}
...
function transactionStatus(client, request) {
  return client.query(graphql_star.TransactionStatusQuery, { request });
}
...
var SessionClient = class _SessionClient extends AbstractClient {
  ...
  logout() {
    return this.getAuthenticatedUser().asyncAndThen(({ authenticationId }) => revokeAuthentication(this, { authenticationId })).andTee(
      () => types_star.ResultAsync.fromPromise(this.storage.reset(), (err3) => UnexpectedError.from(err3))
    );
  }
  /**
   * {@inheritDoc AbstractClient.isPublicClient}
  ...
  waitForTransaction = (txHash) => {
    return types_star.ResultAsync.fromPromise(this.pollTransactionStatus(txHash), (err3) => {
...
var GLOBAL_SPONSORSHIP = types_star.evmAddress(undefined.GLOBAL_SPONSORSHIP);
