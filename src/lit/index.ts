...
import { _createSessionSignatures } from "./session";
import { _mintCapacityToken } from "./capacity";

export const createSessionSignatures = async (litNodeClient: LitNodeClient, Signer: any, capacityTokenId: string) => {

    return await _createSessionSignatures(litNodeClient, Signer, capacityTokenId)
}

export const mintCapacityToken = async (ethersWallet: any, SELECTED_LIT_NETWORK: any) => {

    return await _mintCapacityToken(ethersWallet, SELECTED_LIT_NETWORK) 
}

export const renderwithLitActions =  async (litNodeClient: LitNodeClient, sessionSignatures: SessionSigsMap, protocolInfo: S2SProtocolInfo, notice: any, authorSafeAddress: string, publication: string, contentIds: string[], update: boolean, dev: boolean, debug: boolean, configCid?: string | undefined ) : Promise<any> => {

  if (notice) { notice.setMessage("prepping render ... ") } else  { console.log('prepping render')}
  

  let prep: ExecuteJsResponse = await litNodeClient.executeJs({
...
const rootUpdate = async (litNodeClient: LitNodeClient, sessionSignatures: SessionSigsMap, protocolInfo: S2SProtocolInfo, jsParams: any, maxAttempts = 3) => {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const rootUpdate: any = await litNodeClient.executeJs({
          sessionSigs: sessionSignatures,
